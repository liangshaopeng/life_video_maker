...
def layout_stat(f,tt,dur,A,s):
    ...
    if aa>0:
        ...
        L=txt_layer(str(val),font("n",s.get("num_size",300)),C("white"),stroke=s.get("num_stroke",6),sfill=(0,0,0))
...
def layout_spotlight(f,tt,dur,A,s):
    ...
    if aa>0:
        L=txt_layer(s["big"],font("b",s.get("big_size",148)),C(s.get("big_color","gold")),
                    stroke=s.get("big_stroke",6),sfill=C("navy"))
        blit(f,L,W//2,s.get("big_y",330),lerp(.7,1,e_back(pa)),aa*A)
        if s.get("name"): ctext(d,(W//2,s.get("name_y",452)),s["name"],font("b",s.get("name_size",54)),C("white"),stroke=3,sfill=C("navy"))
