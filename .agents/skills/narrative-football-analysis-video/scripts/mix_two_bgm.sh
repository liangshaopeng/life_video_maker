#!/bin/bash
# Mix two BGM tracks: intro music until a story switch, then main music, with sidechain ducking under narration.
# Usage: mix_two_bgm.sh <input.mp4> <intro.mp3> <main.mp3> <out.mp4> [switch=12.60] [intro_start=0] [main_start=0] [intro_vol=0.20] [main_vol=0.16]
set -euo pipefail
