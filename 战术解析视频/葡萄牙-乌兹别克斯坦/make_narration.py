...
HOMO=cfg.get("tts_homophones",{})          # 展示词->朗读词(同音异形):强制 edge-tts 读音,字幕仍显示展示词
                                           # 例 {"铁子":"帖子"}:朗读"帖子"(tiě·zi,3声铁+轻声子),字幕写"铁子"
def to_spoken(s):
    for disp,spk in HOMO.items(): s=s.replace(disp,spk)
    return s
def to_display(s):
    for disp,spk in HOMO.items(): s=s.replace(spk,disp)
    return s
PUNC="，,。！!？?、；;：:"
STRONG="。！？!?"                            # 句末强停顿
SOFT="，,、；;：:"                            # 句中软停顿
...
def _merge_lines(clauses,maxlen):
    """贪心把小句并成更完整的字幕行:句末强停顿处尽量断,但把过短碎块(<4字)吸进相邻行,避免'到这儿/而梅西'这种碎片。"""
    lines=[]
    for p in clauses:
...
for i,seg in enumerate(cfg["segments"],1):
    text=seg["text"]; spoken=to_spoken(text); mp3=os.path.join(AUD,f"seg{i}.mp3")
    ...
    for _try in range(6):                       # edge-tts 偶发网络失败 -> 重试
        r=subprocess.run(["python3","-m","edge_tts","--voice",VOICE,"--rate",RATE,"--pitch",PITCH,
            "--volume",VOLUME,"--text",spoken,"--write-media",mp3,"--write-subtitles",raw],
    ...
    fine=[(st,min(en,d),to_display(t)) for st,en,t in fine]   # 朗读词->展示词(同音回写):字幕显示原词
    write_srt(vtt,fine)
