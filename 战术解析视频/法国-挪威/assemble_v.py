    ...
    if freeze is not None:                       # 定格:抽一帧 loop;输入 0=still 1=audio 2=overlay
        ...
        if cuts:
            ...
            for ci,cut in enumerate(cuts):
                cstart=cut["start"]; cdur=cut["dur"]; cspeed=cut.get("speed",1.0)
                spd=f"setpts={1.0/cspeed:.4f}*PTS," if abs(cspeed-1.0)>1e-3 else ""
                pre+=(f"[0:v]trim=start={cstart}:duration={cdur:.3f},setpts=PTS-STARTPTS,"
                      f"{spd}fps=30,setsar=1[vcut{ci}];")
                parts.append(f"[vcut{ci}]")
            pre+=f"{''.join(parts)}concat=n={len(parts)}:v=1:a=0[vs];"
        else:
        ...
        nb=len(fp.get("blur",[]))
        if cuts:
            desc="+".join(f"{c['start']}s/{c['dur']}s x{c.get('speed',1.0)}" for c in cuts)
            print(f"→ clip{i}({tl['id']}) cuts[{desc}] {clip:.1f}s"+(f" blur×{nb}" if nb else ""))
        else:
