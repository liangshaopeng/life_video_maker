...
# 自检:打印开头若干 chunk 的声纹判定 + 英文,肉眼核对聚类是否标反
# (开场通常是主持 H 自我介绍/报内容;若这里 H/C 整体颠倒,说明 host_cluster 选反了——
#  极少见,真发生就把 spk_of_cluster 的 H/C 对调,或调大开场判定窗口)
print("\n=== 自检(开头 chunk 的声纹判定,对照原片核对)===")
for x in chunk_spk[:14]:
    print(f" chunk{x['id']:>3} 声纹{x['spk']}(conf {x['conf']})  {x['en']}")
